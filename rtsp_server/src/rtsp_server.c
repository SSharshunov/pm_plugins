...
static void send_audio_packet_cb(evutil_socket_t fd, short events, void *arg) {
    ...
    if (ctx->i * AUDIO_SAMPLES_PER_PACKET >= src_plugins_media_audio_g711a_len) {
        event_del(ctx->ev);
        (*ctx->streams_playing)--;
...
int print_bytes(const char *buf, const uint32_t size) {
    uint32_t edge = 20;
    uint32_t new_size = 0;
    if (size >= edge) {
        new_size = edge;
    ...
    {
        // log_trace("%#02x ", buf[i]);
        // log_trace("%02hhX ", buf[i]); // Prints "0A"
        printf("0x%02hhx ", buf[i]); // Prints "0x0a"
    }
    printf("\n");
    return 0;
}
...
static SmolRTSP_Droppable play_video(
    ...
    U8Slice99 *video = (U8Slice99 *)rqueue_read(rb);
    assert(video);
...
static void send_video_packet_cb(evutil_socket_t fd, short events, void *arg) {
    ...
    const size_t start_code_len = ctx->start_code_tester(*video);
    if (0 == start_code_len) printf("send_video_packet_cb again start_code_len: %zu\n", start_code_len);

    ctx->nalu_start = video->ptr + start_code_len;

    const SmolRTSP_NalUnit nalu = {
        .header = SmolRTSP_NalHeader_H264(
            SmolRTSP_H264NalHeader_parse(ctx->nalu_start[0])),
        // .payload = U8Slice99_new(test + start_code_len + 1, len_b - start_code_len - 1),
        // .payload = U8Slice99_new(tmp_rgbuf.buffer + start_code_len + 1, len_b - start_code_len - 1),
        // .payload = U8Slice99_from_ptrdiff(ctx->nalu_start + 1, video.ptr),
        .payload = U8Slice99_new(video->ptr + start_code_len + 1, video->len - start_code_len - 1),
    ...
    free(video->ptr); video->ptr = NULL;
    free(video); video = NULL;
    return;
}

static void send_video_packet_cb2(evutil_socket_t fd, short events, void *arg) {
    (void)fd;
    (void)events;
...
// }
void some_func() {
    log_info("I am some_func");
}

#if 0

void send_video_test_cb(evutil_socket_t fd, short events, void *arg) {
    (void)fd;
    (void)events;
    (void)arg;
...
//     }
    ...
    int remain = src_plugins_media_audio_g711a_len - done_bytes;
    // log_info("I am send_video_test_cb, remain: %d, done: %d", remain, done_bytes);
    // print_bytes(src_plugins_media_video_h264, 32);
    if (remain < BUF) {
        // print_bytes(src_plugins_media_video_h264 + remain, remain);
        rbuf_write(rb, src_plugins_media_video_h264 + remain, remain);
        done_bytes = 0;
    } else {
        // print_bytes(src_plugins_media_video_h264 + done_bytes, BUF);
        rbuf_write(rb, src_plugins_media_video_h264 + done_bytes, BUF);
        done_bytes = done_bytes + BUF;
        // done_bytes = 0;
    }

    // if (done_bytes + BUF >= src_plugins_media_audio_g711a_len) {

    //     done_bytes = 0;
    // } else if (src_plugins_media_audio_g711a_len - done_bytes) {

    // }
    // rbuf_write(rb, src_plugins_media_video_h264[done_bytes], BUF);
    // if (done_bytes < src_plugins_media_video_h264_len) {
    //     done_bytes = done_bytes + BUF;
    // } else {
    // }
}

static void some_func_cb(evutil_socket_t fd, short events, void *arg) {
    hashtable_t *plugin_config = (hashtable_t *)arg;

    // char* host = (char*)get_config_value(plugin_config, "host");
    // if (ping(host) < 0)
    //     log_warn("network (%s) is down", host);
    // else log_info("network (%s) is ok", host);
    log_info("I am some_func_cb");
}
#endif
...
plugin_t *init(struct event_base *base, hashtable_t *plugin_config) {
    srand(time(NULL));
    assert(base);

    int port = SMOLRTSP_DEFAULT_PORT;

    char* tmp = (char*)get_config_value(plugin_config, "port");
...
//     };
    ...
    // rb = rbuf_create(src_plugins_media_video_h264_len);
    // if (rb)
    //     log_info("Created a new rbuf.");
    // else
    //     log_warn("Cant' create a new rbuf handler");
    ...
    plugin->name = "rtsp_server";
    log_info("Hello, I am a shared library %s", plugin->name);
    // plugin->type = FUNC_VOID;
    plugin->func_ptr = &some_func;
    plugin->argc = 0;
    plugin->instruction = "play rtsp";
    plugin->user = (void*)listener;
    assert(plugin->user);

#if 0
    plugin->events = event_new(
        base, -1, EV_PERSIST | EV_TIMEOUT, send_video_test_cb, (void *)NULL);
    assert(plugin->events);

    g_plugin = plugin;

    if (event_add(plugin->events, &(const struct timeval){
        .tv_sec = 0,
        .tv_usec = 1e6 / VIDEO_FPS,
    }) < 0) {
        event_free(plugin->events);
        free(plugin);
        plugin = NULL;
...
#endif
#if 0
    // struct event *network_availability_handler;
    plugin->events = event_new(
        base, -1, EV_PERSIST | EV_TIMEOUT, some_func_cb, (void *)plugin_config);
    assert(plugin->events);
...
int plugin_destroy(plugin_t *plugin) {
    if (plugin->events) { log_trace("Free plugin->events"); event_free(plugin->events); }
    if (plugin->user) {
        log_trace("Free plugin->user");
        struct evconnlistener *listener = (struct evconnlistener *)plugin->user;
        evconnlistener_free(listener);
    }
    // if (rb) { log_trace("Free rb"); rbuf_destroy(rb); }
    return 0;
}
